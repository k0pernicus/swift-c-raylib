...
#define RL_DEFAULT_SHADER_UNIFORM_NAME_BONEMATRICES                            \
  "boneMatrices" // Bound by default to shader location:
                 // RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEMATRICES
#endif
...
void rlVertex3f(float x, float y, float z) {
  ...
  // RL_QUADS come in groups of 4 vertices Checking current draw.mode when a new
  // vertex is required and finish the batch only if the draw.mode
...
void rlSetTexture(unsigned int id) {
  if (id == 0) {
    ...
    rlDisableTexture();
#else
    // NOTE: If quads batch limit is reached, force a draw call and next batch
    // starts
    if (RLGL.State.vertexCounter >=
        RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
                .elementCount *
...
void rlDrawRenderBatch(rlRenderBatch *batch) {
  ...
  if (RLGL.State.vertexCounter > 0) {
    ...
    // TODO: If no data changed on the CPU arrays there is no need to re-upload
    // data to GPU, a flag can be used to detect changes but it would imply
    // keeping a copy buffer and memcmp() both, does it worth it?
    ...
    // GPU to finish its job To avoid waiting (idle), glBufferData() can bee
    // called first with NULL pointer before glMapBuffer() Doing that, the
...
#if defined(GRAPHICS_API_OPENGL_33)
          // The number of indices to be processed needs to be defined:
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format,
  ...
  if (mipmapCount > 1) {
    ...
    // Define the maximum number of mipmap levels to be used, 0 is base texture
    // size
...
void *rlReadTexturePixels(unsigned int id, int width, int height, int format) {
  ...
  // with texture, glReadPixels() Using Option 1, care for texture format on
  // retrieval NOTE: This behaviour could be conditioned by graphic driver...
...
unsigned int rlLoadShader(const char *code, int type) {
  ...
  if (success == GL_FALSE) {
    ...
    if (maxLength > 0) {
      ...
      TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Compile error: %s", shaderId,
               log);
      RL_FREE(log);
    }
    ...
    case GL_VERTEX_SHADER:
      TRACELOG(RL_LOG_INFO,
               "SHADER: [ID %i] Vertex shader compiled successfully", shaderId);
      break;
    case GL_FRAGMENT_SHADER:
      TRACELOG(RL_LOG_INFO,
...
unsigned int rlLoadShaderProgram(const char *vsCode, const char *fsCode) {
  ...
  if ((vertexShaderId == RLGL.State.defaultVShaderId) &&
    ...
    name);
            ...
            name[namelen] = 0;
            TRACELOG(RL_LOG_DEBUG, "SHADER: [ID %i] Active uniform (%s) set at
    location: %i", id, name, glGetUniformLocation(id, name));
        }
    }
    */
  }
...
unsigned int rlLoadShaderProgramEx(unsigned int vsId, unsigned int fsId) {
  ...
  // attribute variable name that is never used; if some attrib name is no found
  // on the shader, it locations becomes -1
  ...
  glBindAttribLocation(programId, RL_DEFAULT_SHADER_ATTRIB_LOCATION_BONEINDICES,
  ...
  if (success == GL_FALSE) {
    ...
    if (maxLength > 0) {
      ...
      TRACELOG(RL_LOG_WARNING, "SHADER: [ID %i] Link error: %s", programId,
               log);
      RL_FREE(log);
    }
...
void rlUnloadShader(unsigned int id) {
  ...
  TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] Unloaded shader data from VRAM (GPU)",
           id);
#endif
}
